...
print("="*60)
print("SIFT匹配 (排除xzc, 27类)")
print("="*60)

finger_train = {}  # finger -> list of (upsampled_u8, mask)
finger_test = {}
finger_source = {}
...
for finger in sorted(os.listdir(base1)):
    ...
    for p in imgs_paths[:70]:
        ...
        if img is not None:
            ...
            up = upsample_2x(enhanced)
            mask = generate_gmfs_mask(up)
            train_list.append((up, mask))
    for p in imgs_paths[70:]:
        img = load_img(p)
        if img is not None:
            enhanced = clahe_enhance(img)
...
print(f"  Total: {n_classes} classes")
...
for finger in fingers:
    ...
    templates[finger] = [train[i] for i in chosen]
...
for fi, finger in enumerate(fingers):
    ...
    for test_frame in test:
        t_img, t_mask = test_frame
        # Genuine
        sims = [sift_match(t_img, t_mask, tmpl[0], tmpl[1]) for tmpl in templates[finger]]
        gs = max(sims)
        ...
        for other in fingers:
            if other == finger:
                continue
            sims_o = [sift_match(t_img, t_mask, tmpl[0], tmpl[1]) for tmpl in templates[other]]
            impostor_scores.append(max(sims_o))

    if (fi+1) % 5 == 0 or fi == 0:
        print(f"  [{fi+1}/{n_classes}] elapsed={time.time()-t1:.0f}s")

genuine_scores = np.array(genuine_scores, dtype=float)
impostor_scores = np.array(impostor_scores, dtype=float)

print(f"\nEvaluation done in {time.time()-t1:.0f}s")

# ============================================================
# 结果
# ============================================================
print(f"\n{'='*60}")
print(f"Results: SIFT 27类 (排除xzc)")
print(f"{'='*60}")

print(f"\nGenuine:  n={len(genuine_scores)}, mean={genuine_scores.mean():.2f}, "
      f"std={genuine_scores.std():.2f}, min={genuine_scores.min():.0f}")
print(f"Impostor: n={len(impostor_scores)}, mean={impostor_scores.mean():.2f}, "
      f"std={impostor_scores.std():.2f}, max={impostor_scores.max():.0f}")
...
for group_name, prefix in [("无贴屏", "wtp_"), ("不贴屏", "btp_")]:
    group_f = [f for f in fingers if f.startswith(prefix)]
    g_gen, g_imp = [], []
    for finger in group_f:
        test = finger_test[finger]
        for test_frame in test:
            t_img, t_mask = test_frame
            sims = [sift_match(t_img, t_mask, tmpl[0], tmpl[1]) for tmpl in templates[finger]]
            g_gen.append(max(sims))
            for other in group_f:
                if other == finger:
                    continue
                sims_o = [sift_match(t_img, t_mask, tmpl[0], tmpl[1]) for tmpl in templates[other]]
                g_imp.append(max(sims_o))
    ...
    print(f"    gen: mean={g_gen.mean():.2f}, min={g_gen.min():.0f}")
    print(f"    imp: mean={g_imp.mean():.2f}, max={g_imp.max():.0f}")

# 逐手指
print(f"\n{'='*60}")
print("逐手指 genuine 排名")
print("="*60)
stats_list = []
for finger in fingers:
...
for i, (fn, src, gm, gmin, n) in enumerate(stats_list):
    print(f"  {i+1}. {fn} [{src}]: mean={gm:.1f}, min={gmin:.0f}, n={n}")

print(f"\n{'='*60}")
print("总结对比")
print("="*60)
print(f"  SIFT 27类 (排除xzc) EER = {eer*100:.4f}%")
