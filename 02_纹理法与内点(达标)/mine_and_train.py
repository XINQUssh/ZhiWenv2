# -*- coding: utf-8 -*-
"""
精度改进: 用【真实跨帧对应】重训描述子(替代纯增广) + SOSNet二阶相似度正则。
步骤: ①用当前描述子(texdesc.pth)对齐同指帧对, RANSAC内点对=真实对应点;
      ②收集真实对应patch对作正样本; ③HardNet(批内最难负)+SOS 重训 → texdesc_v2.pth
"""
...
CURM=os.environ.get('CURM','f:/1111/指纹/models_texdesc/texdesc.pth')
OUTM=os.environ.get('OUTM','f:/1111/指纹/models_texdesc/texdesc_v2.pth')
cur=Desc().to(dev); cur.load_state_dict(torch.load(CURM,map_location=dev)); cur.eval()
print(f"mining-with={os.path.basename(CURM)} -> {os.path.basename(OUTM)}"); sys.stdout.flush()
...
def align_corr(fa,fb):  # 返回真实对应点对 (srcA, dstB), 经RANSAC+变换约束
    p1,d1=fa; p2,d2=fb
    ...
    return src[mk,0,:], dst[mk,0,:]  # (N,2),(N,2) 真实对应
...
print("load + mine correspondences (all-pairs)..."); sys.stdout.flush(); t0=time.time()
frames=[]; corrs=[]; centers=[]   # corrs: (fiA,xA,yA,fiB,xB,yB); centers: (fi,x,y) 给增广正样本
fcnt=0
for base,pre,reg in [(b1,'wtp','Rgd1245'),(b2,'btp','Rgd1237')]:
    for fg in sorted(os.listdir(base)):
        ...
        sel=list(range(0,70,5))  # 14帧/指
        loc=[]
        for i in sel:
            norm,m=prep(load_img(ps[i])); fi=len(frames); frames.append(norm); loc.append((fi,dense(norm,m)))
            pts=loc[-1][1][0]
            if len(pts):
...
net=Desc().to(dev); opt=torch.optim.AdamW(net.parameters(),lr=5e-4,weight_decay=1e-4)
B=384; ITERS=5000; random.seed(1); np.random.seed(1); torch.manual_seed(1)
print("train (real corr + 增广混合, HardNet批内最难负, 防崩)..."); sys.stdout.flush()
for it in range(ITERS):
    A=np.zeros((B,1,PS,PS),np.float32); P=np.zeros((B,1,PS,PS),np.float32)
    for k in range(B):
        if random.random()<0.6 and corrs:      # 真实跨帧对应
    ...
    if (it+1)%1000==0:
        torch.save(net.state_dict(),OUTM)  # checkpoint防崩
torch.save(net.state_dict(),OUTM)
print(f"saved texdesc_v2.pth t={time.time()-t0:.0f}s\nDone.")
