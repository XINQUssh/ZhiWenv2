"""
SimCLR预训练 + ArcFace微调 — 完整30类验证
- 无贴屏 18类 (Rgd1245)
- 不贴屏 12类 (Rgd1237)
- Set Transformer (FrameEncoder + Attention Aggregator)
- SimCLR自监督预训练 → ArcFace有监督微调
"""
...
for finger in sorted(os.listdir(base1)):
    ...
    for p in imgs_paths:
        ...
        loaded.append(img)
    finger_data[f"wtp_{finger}"] = loaded
    finger_labels[f"wtp_{finger}"] = fi
    finger_source[f"wtp_{finger}"] = "无贴屏"
    fi += 1

n_wtp = fi
print(f"  无贴屏: {n_wtp} classes loaded")
...
for finger in sorted(os.listdir(base2)):
    ...
    for p in imgs_paths:
        ...
        img = (img - img.mean()) / (img.std() + 1e-6)
        loaded.append(img)
    finger_data[f"btp_{finger}"] = loaded
    finger_labels[f"btp_{finger}"] = fi
    finger_source[f"btp_{finger}"] = "不贴屏"
    fi += 1
...
for f in fingers:
    print(f"  {f}: {len(finger_data[f])} frames [{finger_source[f]}]")
...
class FrameSetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frames = np.stack([pool[i] for i in chosen], axis=0)
...
# ---------- Phase 1: SimCLR 预训练 ----------
print("\n" + "="*60)
print("Phase 1: SimCLR Pre-training (30 classes)")
print("="*60)
...
t0 = time.time()
print(f"Pre-training... ({len(pretrain_ds)} samples)")
...
# ---------- Phase 2: ArcFace 微调 ----------
print("\n" + "="*60)
print("Phase 2: ArcFace Fine-tuning (30 classes)")
print("="*60)
...
t1 = time.time()
print(f"Fine-tuning... ({len(ft_ds)} samples)")
for epoch in range(80):
...
for test_n_frames in [15, 20, 25]:
    ...
    for finger in fingers:
        imgs = finger_data[finger]
        train_imgs = imgs[:70]
        test_imgs = imgs[70:]
        enroll_embs[finger] = [get_set_emb(train_imgs, test_n_frames) for _ in range(K)]
        verify_embs[finger] = [get_set_emb(test_imgs, test_n_frames) for _ in range(K)]
...
# 跨组分析: 无贴屏 vs 不贴屏 之间的impostor分数
print("\n  跨组impostor分析 (无贴屏 vs 不贴屏):")
cross_impostor = []
for wf in wtp_fingers:
    for bf in btp_fingers:
        cross_impostor.append(topk_avg(enroll_embs[wf], verify_embs[bf], 5))
print(f"    跨组impostor: mean={np.mean(cross_impostor):.4f}, max={np.max(cross_impostor):.4f}, std={np.std(cross_impostor):.4f}")
