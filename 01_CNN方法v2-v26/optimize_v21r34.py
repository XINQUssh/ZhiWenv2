"""
V21: Hybrid — Layer4全局嵌入 + DELF注意力局部选点
核心思路 (吸取V18b/V19/V20教训):
  V18b: layer4+GAP全局嵌入强(EER=1.41%), 但局部匹配用全部182点均匀网格
  V19: RANSAC在粗网格上无效(impostor也67内点)
  V20: 去layer4→全局嵌入退步(1.82%), 但注意力选点有效(dense 8%→5.8%)

  V21: 两全其美
  1. 保留layer4+GAP做全局嵌入 (V18b的强项, 不动)
  2. 在layer3加DELF注意力模块, 学习哪些空间位置最有判别性
  3. 注意力通过辅助ArcFace训练: 注意力加权池化→分类
  4. 推理: 注意力选top-K关键点做局部匹配 (非均匀网格)
  5. 参数量≈11.5M + 33K(attention) + 132K(attn_proj) ≈ 11.7M
"""
...
print(f"{'='*70}")
print(f"V21: Hybrid — Layer4 Global + DELF Attention Local")
print(f"{'='*70}")
...
USE_AMP = True
N_TEMPLATES = 20
MODEL_SAVE_DIR = 'f:/1111/指纹/models_v21r34/'
...
def train_single_model(seed):
    ...
    save_path = os.path.join(MODEL_SAVE_DIR, f'v21r34_seed{seed}.pth')
...
print(f"  V20 DELF (no layer4):    EER = 1.5967%")
print(f"  V21 best:                EER = {best_eer*100:.4f}%")
