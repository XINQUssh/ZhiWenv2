# -*- coding: utf-8 -*-
"""
迭代3: 全局+局部联合集成 + 分数级融合(最后的零训练杠杆)。
全局: V21+V18b(单) / V25a+V26b+V21(跨) 嵌入加权融合(无TTA, iter2证明TTA有害)。
局部: 各模型local_head的dense_all(top70%)分数, 跨模型平均, 与全局按α融合, 再tnorm。
基线: 单场景iter1=1.107%, 跨场景iter2=5.18%。
"""
...
def load_family(cls,d,pre):
    ...
    return out
FAM={'V18b':load_family(DenseEnc,'models_v18b','v18b'),'V21':load_family(HybridEnc,'models_v21','v21'),
     'V25a':load_family(HybridEnc,'models_v25a','v25a'),'V26b':load_family(HybridEnc,'models_v26b','v26b')}
print("loaded"); sys.stdout.flush()

@torch.no_grad()
def feats(fams, frame):
    """返回 {fam: (g_list[3], ld_list[3])}, g归一化(512,), ld (182,64)归一化。"""
    out={}
    for fam in fams:
        gs,lds=[],[]
        for m in FAM[fam]:
            ...
            lds.append(F.normalize(torch.mean(torch.stack(le),dim=0),dim=1).cpu().numpy())  # (182,64)
        out[fam]=(gs,lds)
    return out
...
for base,pre,reg in [(b1,'wtp','Rgd1245'),(b2,'btp','Rgd1237')]:
    for fg in sorted(os.listdir(base)):
        ...
        k=f'{pre}_{fg}'
        tmpl[k]=[feats(FAMS,prep(load_img(ps[i]))) for i in random.sample(range(70),10)]
...
def build_local_tmpl(lfams):
    """每类每模型: 拼接所有模板帧的局部描述子 → {cid:{fam:[T_m (Nt,64) x3]}}"""
    LT={}
    for cid in ids:
        LT[cid]={}
        for fam in lfams:
            per_m=[]
            for mi in range(3):
                parts=[t[fam][1][mi] for t in tmpl[cid]]   # each (182,64)
                per_m.append(np.concatenate(parts,axis=0))  # (10*182,64)
            LT[cid][fam]=per_m
    return LT

def run(probes, gweights, lfams, alpha, tnorm=False, LT=None):
    GT={cid:[fuse_g(e,gweights) for e in tmpl[cid]] for cid in ids}
    g,i=[],[]
    for gt,plist in probes.items():
        for e in plist:
            pe=fuse_g(e,gweights)
            sc={}
            for cid in ids:
                ...
                if alpha>0:
                    ...
                    for fam in lfams:
                        for mi in range(3):
                            Q=e[fam][1][mi]                 # (182,64)
                            T=LT[cid][fam][mi]              # (Nt,64)
                            S=Q@T.T; mx=S.max(axis=1)
                            k=max(1,int(0.7*len(mx))); ls.append(np.sort(mx)[-k:].mean())
                    lsc=float(np.mean(ls))
                    val=alpha*lsc+(1-alpha)*gs
                else:
                    val=gs
                sc[cid]=val
            for cid in ids:
...
print(f"\n{'='*64}\nEVAL A single-session: global+local fusion  (iter1 best=1.107%)\n{'='*64}")
gwA={'V21':1,'V18b':1}; LTA=build_local_tmpl(['V21','V18b'])
for a in [0.0,0.2,0.3,0.5]:
    print(f"  V21+V18b  alpha_local={a}: raw={run(test,gwA,['V21','V18b'],a,False,LTA)*100:.4f}%  "
          f"tnorm={run(test,gwA,['V21','V18b'],a,True,LTA)*100:.4f}%"); sys.stdout.flush()

print(f"\n{'='*64}\nEVAL B cross-session: global+local fusion  (iter2 best=5.18%)\n{'='*64}")
gwB={'V25a':2,'V26b':1,'V21':1}; LTB=build_local_tmpl(['V25a','V26b'])
for a in [0.0,0.2,0.3,0.5]:
    print(f"  V25a+V26b+V21 / local(V25a+V26b) alpha={a}: "
          f"raw={run(cross,gwB,['V25a','V26b'],a,False,LTB)*100:.4f}%  "
          f"tnorm={run(cross,gwB,['V25a','V26b'],a,True,LTB)*100:.4f}%"); sys.stdout.flush()
print("\nDone.")
