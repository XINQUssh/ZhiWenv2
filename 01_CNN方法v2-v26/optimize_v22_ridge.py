"""
V22: 指纹特有纹理特征 — 脊线方向场 + V21混合架构
核心改进:
  V14-V21都把指纹当通用图像处理, 没有利用指纹特有的脊谷结构

  V22加入指纹域知识:
  1. 4通道输入 (替代1通道):
     - Ch0: 预处理后原始图像
     - Ch1: cos(2θ) — 脊线方向X分量 (θ为局部脊线角度)
     - Ch2: sin(2θ) — 脊线方向Y分量
     - Ch3: coherence — 脊线方向可靠度 (0=无方向, 1=强方向)
  2. 保留V21架构: layer4全局 + 注意力辅助训练 + 局部描述符
  3. 方向感知增强: 翻转时同步变换方向通道

  为什么用cos(2θ)/sin(2θ):
  脊线方向有π模糊性(0°和180°是同一方向), 2θ映射消除此模糊
"""
...
def compute_ridge_orientation(img_float, mask, smooth_sigma=4.0, smooth_ksize=15):
    """Estimate ridge orientation field using gradient-based method.
    Returns cos(2θ), sin(2θ), coherence — all float32, masked.
    """
    ...
    gxx_s = cv2.GaussianBlur(gxx, (smooth_ksize, smooth_ksize), smooth_sigma)
    gyy_s = cv2.GaussianBlur(gyy, (smooth_ksize, smooth_ksize), smooth_sigma)
    gxy_s = cv2.GaussianBlur(gxy, (smooth_ksize, smooth_ksize), smooth_sigma)

    theta2 = np.arctan2(2 * gxy_s, gxx_s - gyy_s)
    cos2t = np.cos(theta2).astype(np.float32)
    sin2t = np.sin(theta2).astype(np.float32)

    denom = gxx_s + gyy_s + 1e-8
    coherence = (np.sqrt((gxx_s - gyy_s)**2 + 4 * gxy_s**2) / denom)
    ...
    return cos2t, sin2t, coherence

def preprocess_frame_v22(img):
    """Returns (4, H, W) float32: [normalized_img, cos2θ, sin2θ, coherence]"""
    ...
    norm_img = (masked.astype(np.float32) - mu) / std
    norm_img[mask == 0] = 0

    cos2t, sin2t, coherence = compute_ridge_orientation(upsampled.astype(np.float32), mask)

    return np.stack([norm_img, cos2t, sin2t, coherence], axis=0)
...
print(f"{'='*70}")
print(f"V22: Ridge Orientation + V21 Hybrid Architecture")
print(f"{'='*70}")
print(f"Loading data (4-channel: img + cos2θ + sin2θ + coherence)...")
sys.stdout.flush()
t_start = time.time()
t_load = time.time()
...
for finger in sorted(os.listdir(base1)):
    ...
    for p in imgs_paths[:70]:
        img = load_img(p)
        if img is not None:
            train_frames.append(preprocess_frame_v22(img))
    for p in imgs_paths[70:]:
        img = load_img(p)
        if img is not None:
            test_frames.append(preprocess_frame_v22(img))
...
class HybridDelfEncoder(nn.Module):
    def __init__(self, embed_dim=512, local_dim=64):
        ...
        self.conv1 = nn.Conv2d(N_INPUT_CH, 64, kernel_size=7, stride=2, padding=3, bias=False)
        gray_w = orig_w.mean(dim=1, keepdim=True)  # (64, 1, 7, 7)
        self.conv1.weight.data[:, 0:1, :, :] = gray_w
        nn.init.kaiming_normal_(self.conv1.weight.data[:, 1:, :, :])
        # Scale down new channels so they don't dominate initially
        self.conv1.weight.data[:, 1:, :, :] *= 0.1
        ...
        self.attention = nn.Sequential(
            nn.Conv2d(256, 128, 1), nn.ReLU(), nn.Conv2d(128, 1, 1), nn.Softplus())
        self.attn_projector = nn.Sequential(
            nn.Linear(256, embed_dim), nn.BatchNorm1d(embed_dim))
        self.local_head = nn.Sequential(
            nn.Conv2d(256, local_dim, kernel_size=1, bias=False), nn.BatchNorm2d(local_dim))
    ...
    def forward(self, x, mode='global'):
        ...
        if mode == 'all':
            ...
            local_desc = F.normalize(self.local_head(x), dim=1)
            feat_norm = F.normalize(x, p=2, dim=1)
            attn_feat = (feat_norm * attn_map).sum(dim=[2, 3])
...
# 数据增强 (4通道, 方向感知)
# ============================================================
def elastic_transform_light_multi(frame, alpha=10, sigma=3):
    """Apply same elastic deformation to all channels."""
    _, h, w = frame.shape
    ...
    for c in range(frame.shape[0]):
        result[c] = cv2.remap(frame[c], map_x, map_y, cv2.INTER_LINEAR, borderValue=0)
    return result

def warp_affine_multi(frame, M, size):
    """Apply affine transform to all channels."""
    result = frame.copy()
    for c in range(frame.shape[0]):
        result[c] = cv2.warpAffine(frame[c], M, size, borderValue=0)
    return result

def augment_frame_multi(frame):
    """Augment 4-channel frame with orientation-aware transforms."""
    f = frame.copy()  # (4, H, W)
    h, w = f.shape[1], f.shape[2]

    # Polarity inversion (image channel only)
    if random.random() < 0.5:
        f[0] = -f[0]

    # Noise (image channel only)
    f[0] += np.random.randn(h, w).astype(np.float32) * 0.03

    # Horizontal flip — negate sin(2θ) to maintain orientation consistency
    if random.random() < 0.3:
        f = f[:, :, ::-1].copy()
        f[2] = -f[2]  # sin(2θ) → -sin(2θ) under horizontal reflection

    # Shift (all channels)
    if random.random() < 0.5:
        dx, dy = random.randint(-5, 5), random.randint(-5, 5)
        M = np.float32([[1, 0, dx], [0, 1, dy]])
        f = warp_affine_multi(f, M, (w, h))

    # Small rotation (all channels; orientation values approximately preserved for ±3°)
    if random.random() < 0.3:
        angle = random.uniform(-3, 3)
        M = cv2.getRotationMatrix2D((w/2, h/2), angle, 1.0)
        f = warp_affine_multi(f, M, (w, h))

    # Elastic transform (all channels, same deformation field)
    if random.random() < 0.25:
        f = elastic_transform_light_multi(f)

    # Random erasing (image channel only)
...
def augment_frame_mild_multi(frame):
    """Mild augment for local contrastive loss."""
    f = frame.copy()
    h, w = f.shape[1], f.shape[2]
    if random.random() < 0.5:
        f[0] = -f[0]
    f[0] += np.random.randn(h, w).astype(np.float32) * 0.02
    if random.random() < 0.5:
...
class ContrastiveDS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        f1 = augment_frame_mild_multi(frame)
        f2 = augment_frame_mild_multi(frame)
...
USE_AMP = True
N_TEMPLATES = 20
MODEL_SAVE_DIR = 'f:/1111/指纹/models_v22/'
...
def train_single_model(seed):
    ...
    save_path = os.path.join(MODEL_SAVE_DIR, f'v22_seed{seed}.pth')
    ...
    print(f"  4-ch input | Phase1: SimCLR+LocalCL {N_PRETRAIN}ep | Phase2: ArcFace(main+attn)+LocalCL {N_FINETUNE}ep (SWA@{SWA_START})")
    ...
    # Also train conv1 since it has new channels
    for param in model.conv1.parameters():
        param.requires_grad = True
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"    Trainable: {trainable:,} (conv1 + attention + local_head + projectors)")
    sys.stdout.flush()
    ...
    param_groups = [
        ...
        {'params': list(model.attention.parameters()) + list(model.attn_projector.parameters()) +
                   list(arcface_attn.parameters()), 'lr': 1e-4},
            ...
            def __init__(self, finger_data, n_samples=20):
                ...
                for k, frames in finger_data.items():
                    for _ in range(n_samples):
                        ...
                        self.items.append(torch.tensor(f, dtype=torch.float32))
...
# ============================================================
@torch.no_grad()
def extract_features(models, frame_4ch):
    """Extract from 4-channel frame. Polarity inversion only on channel 0."""
    global_embs = []
    local_per_model = []
    attn_per_model = []
    ...
    for model in models:
        ...
        for polarity in [1, -1]:
            x = torch.tensor(frame_4ch, dtype=torch.float32).clone()
            x[0] *= polarity  # only invert image channel
...
for finger in fingers:
    ...
    g_list, l_list = [], []
    for i in indices:
        g, l, _ = extract_features(models_list, frames[i])
        g_list.append(g)
        l_list.append(l)
    ...
    for m in range(len(SEEDS)):
        parts = [l_list[t][m] for t in range(len(l_list))]
...
for fi_idx, probe_finger in enumerate(fingers):
    ...
    for frame in test_frames:
        ...
        for identity in fingers:
            ...
            model_dense_all, model_dense_topk = [], []
            for m in range(len(SEEDS)):
                Q = l_descs[m]
                ...
                topk_idx = Q_attn.topk(min(90, Q.shape[0]))[1]
...
# ============================================================
def apply_tnorm(raw, cid, ps, key):
    imp = [ps[k][key] for k in ps if k != cid]
    return (raw - np.mean(imp)) / (np.std(imp) + 1e-8)

print("Step 3: Evaluating variants..."); sys.stdout.flush()

def collect_scores(all_probes):
    DKEYS = ['dense_all', 'dense_topk']
    ...
    for probe in all_probes:
        ...
        for identity in fingers:
            ...
            for dk in DKEYS:
                ...
                for alpha in [0.3, 0.5, 0.7]:
                    fr = alpha * d + (1 - alpha) * g
                    results[f'fused_{dk}_a{alpha}_raw'][tag].append(fr)
                    imp_f = [alpha * sc[o][dk] + (1-alpha) * sc[o]['global'] for o in sc if o != identity]
                    ft = (fr - np.mean(imp_f)) / (np.std(imp_f) + 1e-8)
                    results[f'fused_{dk}_a{alpha}_tnorm'][tag].append(ft)
            if is_gen:
...
VERBOSE_KEYS = ['global_raw', 'global_tnorm', 'fused_dense_all_a0.5_tnorm',
                'fused_dense_topk_a0.5_tnorm', 'fused_dense_all_a0.7_tnorm']
...
print(f"  V21 hybrid:              EER = 1.1898%")
print(f"  V22 best:                EER = {best_eer*100:.4f}%")
improvement = 1.1898 - best_eer * 100
print(f"\n  IMPROVEMENT vs V21: {improvement:+.4f}%")
