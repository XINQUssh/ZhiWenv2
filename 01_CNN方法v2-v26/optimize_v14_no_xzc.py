"""
V14-NoXZC: 排除xzc文件夹, 用27个类重新训练
基于V14 (ResNet-18 + Sub-center ArcFace K=3 + embed512)
客户要求: xzc手指SIFT匹配也差, 排除后看整体改善
"""
...
for finger in fingers:
    ...
    for frame in test_frames:
        q_embs = get_dual_polarity_emb(models, frame)
        own_sims = [dual_polarity_max_sim(q_embs, t) for t in templates[finger]]
        f_genuine.append(max(own_sims))
    f_genuine = np.array(f_genuine)
    src = finger_source.get(finger, "?")
    per_finger_stats.append((finger, src, f_genuine.mean(), f_genuine.min(), f_genuine.std(), len(f_genuine)))
    print(f"  {finger} [{src}]: n={len(f_genuine)}, mean={f_genuine.mean():.4f}, "
          f"min={f_genuine.min():.4f}, std={f_genuine.std():.4f}")

# 按genuine mean排序
per_finger_stats.sort(key=lambda x: x[2])
print(f"\n  --- 按genuine均值排序 (从低到高, 越低越差) ---")
for i, (fn, src, gm, gmin, gstd, n) in enumerate(per_finger_stats):
    status = "*** WORST ***" if i < 3 else ""
    print(f"  {i+1}. {fn} [{src}]: mean={gm:.4f}, min={gmin:.4f} {status}")
sys.stdout.flush()
...
# ============================================================
print(f"\n{'='*60}")
print(f"V14-NoXZC 总结对比 (27类, 排除xzc)")
print(f"{'='*60}")
print(f"  V14-NoXZC (27类)                       EER = {eer_main*100:.4f}%")
print(f"  V14 (30类)                             EER = 3.8989%")
print(f"  V10 SIFT (30类)                        EER = 1.5200%")
