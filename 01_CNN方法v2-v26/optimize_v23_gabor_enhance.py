"""
V23: Gabor脊线增强预处理 + V21混合架构
核心思路:
  V22证明: 直接加脊线方向通道让网络走捷径, 方向是几何信息非身份信息
  V23换一种利用指纹纹理特性的方式:
  1. 用定向Gabor滤波器增强脊线纹理 (标准指纹预处理技术)
     - 估计局部脊线方向 → 沿该方向施加Gabor → 增强脊谷对比度
     - 这不是给网络额外信息, 而是让现有信息更清晰
  2. 多波长Gabor捕获不同脊线间距 (ridge frequency IS identity-discriminative)
  3. 单通道输入 (避免V22的捷径问题), 架构与V21完全相同
"""
...
def gabor_ridge_enhance(img_uint8):
    """Enhance ridge texture using orientation-aligned Gabor filters.

    1. Estimate local ridge orientation via gradient tensor
    2. Apply 8-orientation Gabor bank at multiple wavelengths
    3. At each pixel, select the response matching local ridge direction
    4. Take max across wavelengths (captures local ridge frequency)
    """
    ...
    gxx = cv2.GaussianBlur(gx * gx, (15, 15), 4.0)
    gyy = cv2.GaussianBlur(gy * gy, (15, 15), 4.0)
    gxy = cv2.GaussianBlur(gx * gy, (15, 15), 4.0)
    theta = 0.5 * np.arctan2(2 * gxy, gxx - gyy)  # [-π/2, π/2]

    # Orientation index for 8-direction bank
    N_ORI = 8
    # theta ∈ [-π/2, π/2], map to [0, N_ORI)
    ori_idx = np.round((theta / np.pi + 0.5) * N_ORI).astype(int) % N_ORI

    # Multi-wavelength Gabor: different ridge spacings
    lambdas = [5.0, 7.0, 9.0]
    best_response = np.zeros((h, w), dtype=np.float32)
    ...
    for lambd in lambdas:
        ...
        for i in range(N_ORI):
            angle = i * np.pi / N_ORI
            kern = cv2.getGaborKernel((15, 15), sigma=3.0, theta=angle,
                                      lambd=lambd, gamma=0.5)
            responses.append(cv2.filter2D(img_f, cv2.CV_32F, kern))
        stack = np.stack(responses, axis=-1)  # (H, W, 8)
        # Select response at local orientation
        oriented_resp = stack[y_idx, x_idx, ori_idx]
        # Take max absolute across wavelengths (best ridge frequency match)
        mask_better = np.abs(oriented_resp) > np.abs(best_response)
        best_response = np.where(mask_better, oriented_resp, best_response)

    return best_response

def preprocess_frame_v23(img):
    """Gabor-enhanced preprocessing: CLAHE → upsample → Gabor enhance → mask → normalize"""
    enhanced = clahe_enhance(img)
    upsampled = upsample_2x(enhanced)
    mask = generate_gmfs_mask(upsampled)

    # Gabor ridge enhancement
    ridge_img = gabor_ridge_enhance(upsampled)

    # Apply mask and normalize
    ridge_img[mask == 0] = 0
    valid = ridge_img[mask > 0]
    if len(valid) > 0:
        mu, std = valid.mean(), valid.std() + 1e-6
    else:
        mu, std = 0, 1
    result = (ridge_img - mu) / std
    result[mask == 0] = 0
...
print(f"{'='*70}")
print(f"V23: Gabor Ridge Enhancement + V21 Hybrid Architecture")
print(f"{'='*70}")
print(f"Loading data (Gabor-enhanced single-channel)..."); sys.stdout.flush()
t_start = time.time()
t_load = time.time()
...
for finger in sorted(os.listdir(base1)):
    ...
    for p in imgs_paths[:70]:
        img = load_img(p)
        if img is not None:
            frame, _ = preprocess_frame_v23(img)
...
class HybridDelfEncoder(nn.Module):
    def __init__(self, embed_dim=512, local_dim=64):
        ...
        self.bn1 = base.bn1; self.relu = base.relu; self.maxpool = base.maxpool
    ...
    def forward(self, x, mode='global'):
        ...
        if mode == 'all':
            ...
            attn_emb = self.attn_projector((feat_norm * attn_map).sum(dim=[2, 3]))
        x4 = self.avgpool(self.layer4(x)).flatten(1)
...
USE_AMP = True
N_TEMPLATES = 20
MODEL_SAVE_DIR = 'f:/1111/指纹/models_v23/'
...
def train_single_model(seed):
    ...
    save_path = os.path.join(MODEL_SAVE_DIR, f'v23_seed{seed}.pth')
    if os.path.exists(save_path):
        ...
        model.eval(); print(f"  Loaded."); sys.stdout.flush()
        return model

    print(f"\n{'='*60}")
    print(f"Training seed={seed}")
    print(f"  Gabor-enhanced input | Phase1: SimCLR+LocalCL {N_PRETRAIN}ep | Phase2: ArcFace(main+attn)+LocalCL {N_FINETUNE}ep (SWA@{SWA_START})")
    ...
    opt1 = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=0.001, weight_decay=1e-3)
    ...
    for epoch in range(N_PRETRAIN):
        model.train(); tl_g, tl_a, tl_l = 0, 0, 0
    ...
    arcface_main = SubCenterArcFace(EMBED_DIM, n_classes, K=3, s=64, m=0.5, label_smoothing=0.1).to(device)
    arcface_attn = SubCenterArcFace(EMBED_DIM, n_classes, K=3, s=64, m=0.5, label_smoothing=0.1).to(device)
    ...
    param_groups = [
        ...
        {'params': list(model.layer3.parameters()) + list(model.local_head.parameters()), 'lr': 5e-5},
        ...
        {'params': list(model.global_projector.parameters()) + list(arcface_main.parameters()), 'lr': 3e-4},
    ]
...
for finger in fingers:
    ...
    for i in indices:
        ...
        g_list.append(g); l_list.append(l)
    template_global[finger] = g_list
    per_model_concat = []
    for m in range(len(SEEDS)):
        per_model_concat.append(torch.cat([l_list[t][m] for t in range(len(l_list))], dim=0))
...
for fi_idx, probe_finger in enumerate(fingers):
    for frame in finger_test[probe_finger]:
        ...
        for identity in fingers:
            ...
            model_dense = []
            for m in range(len(SEEDS)):
                Q = l_descs[m]; T = template_local_concat[identity][m]
                S = torch.mm(Q, T.t())
                max_sim, _ = S.max(dim=1)
                k = max(1, int(0.7 * Q.shape[0]))
                top_v, _ = max_sim.topk(k)
                model_dense.append(top_v.mean().item())
            scores[identity] = {'global': global_score, 'dense': np.mean(model_dense)}
...
def collect_scores(all_probes):
    ...
    for alpha in [0.3, 0.5, 0.7]:
        variants += [f'fused_a{alpha}_raw', f'fused_a{alpha}_tnorm']
    results = {v: {'gen': [], 'imp': []} for v in variants}
    pf_gen = {f: [] for f in fingers}

    for probe in all_probes:
        pf = probe['probe_finger']
        sc = probe['scores']
        for identity in fingers:
            g = sc[identity]['global']; d = sc[identity]['dense']
            is_gen = (identity == pf); tag = 'gen' if is_gen else 'imp'
            ...
            results['dense_raw'][tag].append(d)
            results['dense_tnorm'][tag].append(d_t)
            for alpha in [0.3, 0.5, 0.7]:
                fr = alpha * d + (1-alpha) * g
                results[f'fused_a{alpha}_raw'][tag].append(fr)
                imp_f = [alpha*sc[o]['dense']+(1-alpha)*sc[o]['global'] for o in sc if o != identity]
                results[f'fused_a{alpha}_tnorm'][tag].append(
                    (fr - np.mean(imp_f)) / (np.std(imp_f) + 1e-8))
            if is_gen: pf_gen[pf].append(g)

    for k in results:
        results[k]['gen'] = np.array(results[k]['gen'])
        results[k]['imp'] = np.array(results[k]['imp'])
    return results, pf_gen

results, pf_gen = collect_scores(all_probes)
...
for key in sorted(results.keys()):
    ...
    verbose = key in ['global_raw', 'global_tnorm', 'fused_a0.5_tnorm', 'fused_a0.7_tnorm']
...
print(f"  V22 ridge channels:      EER = 4.9342%")
print(f"  V23 best:                EER = {best_eer*100:.4f}%")
