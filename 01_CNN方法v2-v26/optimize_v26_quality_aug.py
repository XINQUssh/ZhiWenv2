"""
V26: V25a合并训练 + 质量感知过滤 + 跨场景增广 (raw only)
背景: V25a(同手指合并)把跨场景EER从57%降到5.71%(老协议1.54%)。手工方向场/脊线验证无效。
      下一步攻真实瓶颈: 跨场景域偏移 + 新数据垃圾帧(客户拒识表: lwh全拒/dy_R0拒73等)。

在V25a基础上的两处增量(其余完全不变, 评估协议与V21/V25可比):
  1. 跨场景增广 augment_frame_cross: 在原增广上叠加模拟超声跨场景的变化 —
     增益/对比抖动(耦合压力) + 低频偏置场(耦合不均) + gamma非线性 + 更强弹性 + 尺度抖动。
     仅用于Phase2判别训练的强增广视图。
  2. 质量感知过滤: 仅过滤【新数据帧】(extra_train/new_only), 老数据finger_train不动
     → 保持Eval A模板RNG与V21比特级可比。用V25a嵌入在【同手指同场景组内】算一致性,
     剔除拍到不同区域的垃圾帧(按组内一致性, 不会误删真正的跨场景信号) + 前景覆盖度下限。
  DSBN域自适应留作V27(先单独验证质量+增广的增量, 干净归因)。
  仅raw(降噪已证全面更差)。
"""
...
print(f"{'='*70}")
print(f"V26 ({ARM}): merge + quality-filter + cross-aug (ysjz {VARIANT})")
print(f"{'='*70}")
...
def augment_frame_cross(frame):
    """V26: 在强增广基础上叠加模拟超声跨场景的光度/几何变化。"""
    f = frame.copy()
    bg = (frame == 0)
    # --- 光度: 模拟耦合/压力/湿度差异 ---
    if random.random() < 0.5: f = -f
    if random.random() < 0.7: f = f * random.uniform(0.6, 1.5)              # 增益抖动
    if random.random() < 0.5:                                              # 低频偏置场(耦合不均)
        h, w = f.shape
        bias = cv2.GaussianBlur(np.random.randn(h, w).astype(np.float32), (0, 0), 40)
        f = f + bias * random.uniform(0.1, 0.4)
    if random.random() < 0.3:                                              # gamma非线性
        g = random.uniform(0.7, 1.4)
        f = np.sign(f) * (np.abs(f) ** g)
    f = f + np.random.randn(*f.shape).astype(np.float32) * 0.03
    f[bg] = 0                                                              # 几何前先复位背景
    # --- 几何: 模拟按压畸变/位置差异 ---
    h, w = f.shape
    if random.random() < 0.6:
        dx, dy = random.randint(-7, 7), random.randint(-7, 7)
        f = cv2.warpAffine(f, np.float32([[1,0,dx],[0,1,dy]]), (w, h), borderValue=0)
    if random.random() < 0.5:
        ang = random.uniform(-5, 5); sc = random.uniform(0.9, 1.1)
        f = cv2.warpAffine(f, cv2.getRotationMatrix2D((w/2, h/2), ang, sc), (w, h), borderValue=0)
    if random.random() < 0.45: f = elastic_transform_light(f, alpha=20, sigma=4)  # 更强弹性
    if random.random() < 0.3: f = f[:, ::-1].copy()
    if random.random() < 0.15:                                            # 随机擦除
        ea = random.uniform(0.02, 0.06) * h * w; asp = random.uniform(0.5, 2.0)
        eh = min(int(np.sqrt(ea * asp)), h - 1); ew = min(int(np.sqrt(ea / asp)), w - 1)
        if eh > 0 and ew > 0:
            t, l = random.randint(0, h - eh), random.randint(0, w - ew)
            f[t:t+eh, l:l+ew] = 0
    return f
...
class ClassifyDS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frames = self.finger_data[finger]
        frame = frames[random.randint(0, len(frames)-1)]
        f1 = augment_frame_cross(frame)   # V26: 跨场景增广
        f2 = augment_frame_mild(frame)
        return (torch.tensor(f1, dtype=torch.float32).unsqueeze(0),
                torch.tensor(f2, dtype=torch.float32).unsqueeze(0),
...
# ============================================================
# V26 质量感知过滤: 仅过滤新数据帧(extra_train/new_only)
#   用V25a嵌入在【同手指同场景组内】算一致性, 剔除拍到不同区域的垃圾帧 + 低覆盖帧。
#   老数据finger_train不动 → 保持Eval A模板RNG与V21比特级可比。
#   组内一致性(非到老中心) → 不会误删真正的跨场景信号。
# ============================================================
def _coverage(frame):
    return float((frame != 0).mean())

@torch.no_grad()
def _emb_v25a(qmodels, frame):
    es = []
    for m in qmodels:
        for pol in (1, -1):
...
def _quality_filter(frames, qmodels, mad_k=2.5, max_drop_frac=0.30, min_keep=45, cov_floor=0.10):
    """自适应过滤: 组内一致性的MAD离群帧 + 低覆盖帧。
    干净组(MAD小)几乎不丢; 脏组(一致性散)多丢; 上限max_drop_frac, 保底min_keep。"""
    n = len(frames)
    if n < 30:
        return frames, 0
    embs = np.stack([_emb_v25a(qmodels, f) for f in frames])
    centroid = embs.mean(0); centroid /= (np.linalg.norm(centroid) + 1e-8)
    cons = embs @ centroid
    covs = np.array([_coverage(f) for f in frames])
    med = np.median(cons); mad = np.median(np.abs(cons - med)) + 1e-8
    drop = set()
    for i in range(n):
        if covs[i] < cov_floor or cons[i] < med - mad_k * mad:   # 低覆盖 或 统计离群
            drop.add(int(i))
    cap = int(max_drop_frac * n)                                  # 上限: 最多丢max_drop_frac
    if len(drop) > cap:
        drop = set(int(i) for i in sorted(drop, key=lambda j: cons[j])[:cap])
    if n - len(drop) < min_keep:                                 # 保底min_keep
        keep_idx = set(int(i) for i in np.argsort(-cons)[:min_keep])
        drop = set(range(n)) - keep_idx
    kept = [frames[i] for i in range(n) if i not in drop]
    return kept, n - len(kept)

_v25a_paths = [f'f:/1111/指纹/models_v25a/v25a_seed{s}.pth' for s in SEEDS]
if all(os.path.exists(p) for p in _v25a_paths):
    print("\nV26 quality filter: scoring new-data frames with V25a..."); sys.stdout.flush()
    _qmodels = []
    for p in _v25a_paths:
        qm = HybridDelfEncoder(embed_dim=EMBED_DIM, local_dim=LOCAL_DIM).to(device)
        qm.load_state_dict(torch.load(p, map_location=device, weights_only=True))
        qm.eval(); _qmodels.append(qm)
    _tq = time.time(); _total_drop = 0
    for k in list(extra_train.keys()):
        extra_train[k], d = _quality_filter(extra_train[k], _qmodels); _total_drop += d
        if d: print(f"    {k:14s} (merged new): dropped {d}, kept {len(extra_train[k])}")
    for k in list(new_only.keys()):
        new_only[k], d = _quality_filter(new_only[k], _qmodels); _total_drop += d
        if d: print(f"    {k:14s} (new-only):    dropped {d}, kept {len(new_only[k])}")
    del _qmodels; torch.cuda.empty_cache()
    train_combined = {k: finger_train[k] + extra_train.get(k, []) for k in finger_train}
    train_combined.update(new_only)
    print(f"  Quality filter dropped {_total_drop} garbage new-frames, "
          f"time={time.time()-_tq:.0f}s"); sys.stdout.flush()
else:
    print("\nV26 WARNING: V25a models not found, skip quality filter."); sys.stdout.flush()
...
# Final summary
print(f"\n{'='*70}")
print(f"V26 ({ARM}, merge + quality-filter + cross-aug, +ysjz {VARIANT}) FINAL SUMMARY")
print(f"{'='*70}")
...
print(f"  V25a (merge only):        EER = 1.5375%")
improvement = 1.5375 - best_eer * 100
print(f"  IMPROVEMENT vs V25a: {improvement:+.4f}%  (positive = better)")
...
print(f"  V21 frozen baseline:      EER = 56.91% (global_raw, ≈random)")
print(f"  V25a (merge only) best:   EER = 5.7135%")
cross_impr = 5.7135 - cross_eers[cross_best] * 100
print(f"  IMPROVEMENT vs V25a: {cross_impr:+.4f}%  (positive = better)")
