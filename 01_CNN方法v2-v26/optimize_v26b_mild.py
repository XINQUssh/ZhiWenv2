...
USE_QUALITY_FILTER = False   # V26b: 隔离增广效果, 关闭质量过滤(与V25a同数据)
...
def augment_frame_cross(frame):
    """V26b: V25a原增广 + 仅【温和】增益/偏置场(模拟超声耦合差异)。去掉gamma与超强弹性。"""
    f = frame.copy()
    bg = (frame == 0)
    # --- 光度: 温和模拟耦合/压力差异 ---
    if random.random() < 0.5: f = -f
    if random.random() < 0.4: f = f * random.uniform(0.8, 1.2)             # 温和增益
    if random.random() < 0.3:                                             # 温和低频偏置场
        h, w = f.shape
        bias = cv2.GaussianBlur(np.random.randn(h, w).astype(np.float32), (0, 0), 40)
        f = f + bias * random.uniform(0.05, 0.2)
    ...
    if random.random() < 0.3:
        ...
        f = cv2.warpAffine(f, cv2.getRotationMatrix2D((w/2, h/2), ang, 1.0), (w, h), borderValue=0)
    if random.random() < 0.25: f = elastic_transform_light(f)            # 原强度alpha=10
    if random.random() < 0.3: f = f[:, ::-1].copy()
    if random.random() < 0.15:
...
if USE_QUALITY_FILTER and all(os.path.exists(p) for p in _v25a_paths):
