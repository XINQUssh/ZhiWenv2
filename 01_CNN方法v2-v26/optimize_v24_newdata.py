"""
V24: V21架构 + 客户新数据(ysjz)作为额外训练类别
背景:
  客户上传 ysjz.zip(原图) + ysjz_denoised.zip(降噪后), 5人22个手指文件夹, 每folder约100帧.
  用V21模型做身份检验: 新数据同名手指(dy_R0等)与老数据同名类相似度仅0.2-0.4(impostor水平),
  且存在严重域偏移(新数据噪声大/有横条纹) → 新数据全部作为独立新类, 不与老类合并.

设计:
  1. 模型/训练/超参与V21完全一致 (ResNet18 + layer4全局 + DELF注意力辅助 + 局部描述符)
  2. 新数据仅加入训练(全部帧), 不参与评估
  3. 评估协议与V21完全一致(老27类, 70/30分割, 每类20模板, seed42) → EER与V21的1.1898%直接可比
  4. 变体: python optimize_v24_newdata.py raw      → 加入ysjz原图   (models_v24a)
          python optimize_v24_newdata.py denoised → 加入ysjz降噪图 (models_v24b)
  排除: ssh_R0_failure(19帧<50). SSH_L2-half(63帧)保留.
"""
...
print(f"{'='*70}")
print(f"V24 ({ARM}): V21 arch + new ysjz data ({VARIANT}) as extra train classes")
print(f"{'='*70}")
...
for folder in sorted(os.listdir(NEW_BASE)):
    ...
    key = f"new_{folder}"
    train_frames = []
    for p in imgs_paths:          # 全部帧用于训练(无需留测试)
        img = load_img(p)
        if img is not None:
            frame, _ = preprocess_frame(img)
            train_frames.append(frame)
...
n_classes = fi
fingers = eval_fingers             # 下方评估代码遍历的是老27类
print(f"  New data ({VARIANT}): {n_classes-n_eval} train-only classes, time={time.time()-t_new:.0f}s")
...
# ============================================================
print(f"\n{'='*70}")
print(f"Evaluating V24 ({ARM}) — protocol identical to V21 (old {n_eval} classes)")
