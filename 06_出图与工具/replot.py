...
gen=np.load('f:/1111/指纹/scores_gen.npy'); imp=np.load('f:/1111/指纹/scores_imp.npy')
...
ax1.axvline(thr0,ls='--',color='gray'); ax1.set_xlabel('unlock threshold (inliers)')
ax1.legend(loc='center right'); ax1.set_ylim(-0.02,1.02)
ax1.set_title('Matching Performance — transform-constrained inlier matcher',fontsize=11)
...
cols=['thr','FAR','FRR','TAR','gen_acc','gen_rej','imp_acc','imp_rej']
rows=[]
...
ax3.text(0.5,1.02,f'Operating points   (genuine_total={ng},  impostor_total={ni})',
         ha='center',fontsize=10,transform=ax3.transAxes)
fig.text(0.5,0.355,f'*** FAR=0 @ thr={thr0}:  FRR={frr0*100:.2f}%,  TAR={(1-frr0)*100:.2f}%   (all 27 fingers; ~1.1% excluding 2 degraded fingers) ***',
         ha='center',fontsize=10,weight='bold',color='#b00')
plt.savefig('f:/1111/指纹/匹配效果.png',dpi=115,bbox_inches='tight')
print(f'replotted. FAR=0@thr{thr0} FRR={frr0*100:.2f}% imp_max={imp.max()} gen_mean={gen.mean():.0f}')
